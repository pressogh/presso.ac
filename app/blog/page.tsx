...
const getData = async () => {
	const mdFiles = await fetch(`${process.env.BUCKET_PRE_SIGNED_URL}`, { next: { revalidate: 3600 } }).then(async (res) => {
	...
	const posts: PostType[] = await Promise.all(
		mdFiles.map(async (item: { name: string }) => {
			...
			const post = await fetch(`${process.env.BUCKET_PRE_SIGNED_URL}/resume/posts/${encodeURIComponent(folder ? folder : '')}/data.mdx`, { next: { revalidate: 3600 } }).then((res) => res.text());
